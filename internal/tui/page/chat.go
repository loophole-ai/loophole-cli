...
import (
	...
	"github.com/loophole-ai/loophole-cli/internal/completions"
...
func (p *chatPage) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case SetCommandsMsg:
		...
		// Re-initialize completion dialog with updated commands
		cg := completions.NewFileAndFolderContextGroup()
		cmdProvider := completions.NewCommandCompletionProvider(p.commands)
		router := completions.NewCompletionRouter(cg, cmdProvider)
		p.completionDialog = dialog.NewCompletionDialogCmp(router)
		return p, p.completionDialog.Init()
...
func (p *chatPage) sendMessage(text string, attachments []message.Attachment) tea.Cmd {
	if strings.HasPrefix(text, "/") {
		commandTitle := strings.TrimPrefix(text, "/")
		for _, cmd := range p.commands {
			if strings.EqualFold(cmd.Title, commandTitle) {
